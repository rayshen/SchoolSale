...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    static NSString * cellIndentifier=@"ReplyTableViewCell";
    ReplyTableViewCell * cell=[self.tableView dequeueReusableCellWithIdentifier:cellIndentifier];
    if (cell == nil) {
        NSArray *array = [[NSBundle mainBundle]loadNibNamed:@"ReplyTableViewCell" owner:self options:nil];
        cell = [array objectAtIndex:0];
        [cell setSelectionStyle:UITableViewCellSelectionStyleNone];
    }
    
    if (indexPath.row==0) {
        cell.namelabel.text=@"校园精灵";
        cell.tximageview.image=[UIImage imageNamed:@"systemmessage"];
        cell.contentlabel.text=@"校园东西让你更好地体验大学生活的交易乐趣！";
    }
    if (indexPath.row==1) {
        cell.namelabel.text=@"乔布斯";
        cell.tximageview.image=[UIImage imageNamed:@"demo-avatar-jobs"];
        cell.contentlabel.text=@"回复我：呵呵。";
    }
    if (indexPath.row==2) {
        cell.namelabel.text=@"沃茨";
        cell.tximageview.image=[UIImage imageNamed:@"demo-avatar-woz"];
        cell.contentlabel.text=@"回复我：便宜一点肯定好卖。";
    }
